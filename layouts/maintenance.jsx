...
export default function MasterLayout({ title, leftContent, children }) {
  ...
  const renderMainContent = () => {
    return (
      <div className={styles.mainContent}>
        ...
        <div className={styles.rightColumn}>{!isNarrowWindow ? renderRightColumn() : <></>}</div>
      </div>
    )
  }
  ...
  return (
    <>
      <PageTop
        title={title}
        onShowFolderView={onShowFolderView}></PageTop>

      {isNarrowWindow ? (
        <Drawer
          anchor="right"
          open={drawOpenState}
          onClose={() => setDrawOpenState(false)}>
          <div className={styles.drawerContent}>
            <div className={styles.drawerClose}>
              <IconButton
                aria-label="close folder list"
                color="inherit"
                size={isNarrowWindow ? 'large' : 'small'}
                onClick={() => setDrawOpenState(false)}>
                <CloseIcon></CloseIcon>
              </IconButton>
            </div>
            {renderRightColumn()}
