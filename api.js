...
export const createPostAPI = (discussion, text, postAsAdmin, subscribeToDiscussion) =>
  Api.Post(`/folder/${discussion.folderId}/discussion/${discussion.id}/post`, {
    text,
    postAsAdmin,
    subscribeToDiscussion
  })
export const editPostAPI = (discussion, post) =>
  Api.Put(`/folder/${discussion.folderId}/discussion/${discussion.id}/post/${post.id}`, { text: post.text })
export const deletePostAPI = (discussion, post) =>
  Api.Delete(`/folder/${discussion.folderId}/discussion/${discussion.id}/post/${post.id}`)

export const fetchFoldersAPI = () => Api.Get(`/folder`)

export const fetchDiscussionsAPI = (folder, start, size) =>
  Api.Get(`/folder/${folder.id}/discussion?start=${start}&size=${size}`)
export const fetchDiscussionsBeforeAPI = (folder, beforeDate, size) =>
  Api.Get(`/folder/${folder.id}/discussion/before?dt=${encodeURIComponent(beforeDate)}&size=${size}`)

export const fetchDiscussionAPI = (folder, discussionId) => Api.Get(`/folder/${folder.id}/discussion/${discussionId}`)
export const createDiscussionAPI = (folder, title, header, isSubscribed) =>
  Api.Post(`/folder/${folder.id}/discussion`, { title, header, isSubscribed })
export const editDiscussionAPI = (discussion, title, header) =>
  Api.Put(`/folder/${discussion.folderId}/discussion/${discussion.id}`, { title, header })
export const deleteDiscussionAPI = discussion =>
  Api.Delete(`/folder/${discussion.folderId}/discussion/${discussion.id}`)

export const fetchFrontPageAPI = (viewType, start, size) =>
  Api.Get(`/frontpage/${viewType}?start=${start}&size=${size}`)
export const fetchFrontPageSinceAPI = (viewType, sinceDate, size) =>
  Api.Get(`/frontpage/${viewType}/since?dt=${encodeURIComponent(sinceDate)}&size=${size}`)
export const fetchFrontPageBeforeAPI = (viewType, beforeDate, size) =>
  Api.Get(`/frontpage/${viewType}/before?dt=${encodeURIComponent(beforeDate)}&size=${size}`)
...
export const fetchModerationHistoryAPI = (start, size) =>
  Api.Get(`/admin/moderation/history?start=${start}&size=${size}`)

export const fetchPostReportsAPI = (discussion, post) =>
  Api.Get(`/admin/discussion/${discussion.id}/post/${post.id}/report`)
export const fetchPostCommentsAPI = (discussion, post) =>
  Api.Get(`/admin/discussion/${discussion.id}/post/${post.id}/comment`)
export const createAdminCommentAPI = ({ discussionId, postId, vote, body }) =>
  Api.Post(`/admin/discussion/${discussionId}/post/${postId}/report`, { vote, body })

export const lockDiscussionAPI = (discussion, lockState) =>
  Api.Post(`/admin/discussion/${discussion.id}/lock?state=${lockState ? 1 : 0}`)
export const premoderateDiscussionAPI = (discussion, premodState) =>
  Api.Post(`/admin/discussion/${discussion.id}/premoderate?state=${premodState ? 1 : 0}`)
export const adminDeleteDiscussionAPI = (discussion, deleteState) =>
  Api.Post(`/admin/discussion/${discussion.id}/delete?state=${deleteState ? 1 : 0}`)
export const moveDiscussionAPI = (discussion, targetFolder) =>
  Api.Post(`/admin/discussion/${discussion.id}/move?targetFolderId=${targetFolder.id}`)
export const eraseDiscussionAPI = discussion => Api.Delete(`/admin/discussion/${discussion.id}`)

export const blockUserFromDiscussionAPI = (discussion, userId) =>
  Api.Post(`/admin/discussion/${discussion.id}/user/block/${userId}`)
export const unblockUserFromDiscussionAPI = (discussion, userId) =>
  Api.Delete(`/admin/discussion/${discussion.id}/user/block/${userId}`)
...
export const ignoreUserAPI = (ignoreUserId, shouldIgnore) =>
  Api.Put(`/user/ignore/${ignoreUserId}?state=${shouldIgnore ? 1 : 0}`)

export const setDiscussionUnreadAPI = discussion => Api.Delete(`/user/discussion/${discussion.id}/bookmark`)
export const updateCurrentBookmarkAPI = (discussion, bookmarkedPost) =>
  Api.Put(`/user/discussion/${discussion.id}/bookmark`, bookmarkedPost)
...
export const setDiscussionSubscriptionStatusAPI = discussion =>
  Api.Post(`/folder/${discussion.folderId}/discussion/${discussion.id}/subscription`)
export const unsetDiscussionSubscriptionStatusAPI = discussion =>
  Api.Delete(`/folder/${discussion.folderId}/discussion/${discussion.id}/subscription`)
...
export const updateUserSubscriptionFetchOrderAPI = subscriptionFetchOrder =>
  Api.Put(`/user/subscriptions/fetchorder`, { subscriptionFetchOrder })
