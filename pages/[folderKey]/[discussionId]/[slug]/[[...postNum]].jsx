...
import { deleteDiscussion } from '../../../../redux/discussionActions'
...
export default function DiscussionView(props) {
  ...
  useEffect(() => {
    ...
    let shouldShowAddPosts =
      !editDiscussion &&
      (currentDiscussion.postCount == 0 ||
        (posts.length > 0 && posts[posts.length - 1].postNum >= currentDiscussion.postCount))
  ...
  const renderLeftMenu = () => {
    ...
    return (
      <div className="leftButtonMenu">
        <div className="leftButtonMenuInner">
          {posts.length === 0 &&
          currentDiscussion.postCount == 0 &&
          currentDiscussion.createdByUserId == currentUser.id ? (
            <>
              <ActionButton
          ...
          {currentUser && currentUser.isAdmin ? (
            <>
              <ActionButton
                icon={<Build />}
                label="Admin"
                onClick={key => onClickAdminMenu(key)}
                menu={[
  ...
  const renderBreadcrumbs = () => {
    return (
      <Paper
        ...
        <div className="breadcrumb-item">
          <Link href={`/${currentFolder.key}`}>
            <a>{currentFolder.description}</a>
          </Link>
        </div>
      </Paper>
    )
      ...
      <Paper
        ...
        <article className={styles.discussionHeader}>
          <div className={styles.discussionHeaderContent}>
            <Typography
              ...
              dangerouslySetInnerHTML={discussionTitle()}></Typography>
            <Typography
              variant="overline"
              display="block"
              gutterBottom>
              <UserLink
                userId={currentDiscussion.createdByUserId}
                username={currentDiscussion.createdByUsername}
              />
              <span>{` - ${formatDistanceToNow(new Date(currentDiscussion.createdDate))} ago`}</span>
            </Typography>
            <Typography
              variant="body1"
              display="block"
              ...
              dangerouslySetInnerHTML={headerMarkup()}></Typography>
          </div>
        </article>
        ...
        <div className={styles.discussionPostList}>
            ...
            <Alert
              ...
              action={
                <Button
                  color="primary"
                  size="small"
                  onClick={onSetClickToViewNewPosts}>
                  Click to view
                </Button>
              }>{`${
              pendingDicussionUpdateAvailable.postCount - currentDiscussion.postCount
          ...
          {showAddPosts ? (
            <AddPost
              discussion={currentDiscussion}
              isEditing={onIsEditingPostChanged}
            />
          ) : (
            <></>
          )}
  ...
  return (
    <MasterLayout
      title={`JUSTtheTalk - ${currentDiscussion ? currentDiscussion.title : 'Discussion'}`}
      leftContent={renderLeftMenu()}>
      {currentDiscussion ? (
        <Head>
          <meta
            name="description"
            content={currentDiscussion.header}
          />
          <link
            rel="canonical"
            href={`https://justthetalk.co.uk${currentDiscussion.url}`}
          />
          <meta
          ...
          <meta
            property="og:url"
            content={`https://justthetalk.co.uk${currentDiscussion.url}`}
          />
        </Head>
      ) : (
        <></>
      ...
      <div
        ...
        <div
          ...
          style={{
            visibility:
              fetchPostsState === LoadingState.Loaded && posts.length > 0 && !isEditingPost ? 'visible' : 'hidden'
          }}
        ...
        {currentDiscussion && editDiscussion ? (
          <CreateDiscussion
            ...
            onCancelCreateDiscussion={onCancelEditDiscussion}
            onEditedDiscussion={onEditedDiscussion}
            discussion={currentDiscussion}></CreateDiscussion>
        ) : (
          <></>
        )}
      ...
      <ReportPostDialog
        open={Boolean(reportedPost)}
        onClose={onCloseReportDialog}
        post={reportedPost}
        discussion={currentDiscussion}
        folder={currentFolder}></ReportPostDialog>

      <Dialog
        open={openDeleteDiscussionDialog}
        onClose={onCloseDeleteDiscussionDialog}
        ...
        <DialogActions>
          <Button
            onClick={onCloseDeleteDiscussionDialog}
            color="primary">
            Cancel
          </Button>
          <Button
            onClick={onConfirmDeleteDiscussionDialog}
            color="primary"
            autoFocus
            data-test-id="delete-discussion-ok">
            OK
          </Button>
        </DialogActions>
      </Dialog>
      ...
      {currentUser && currentUser.isAdmin && currentDiscussion ? (
        <Dialog
          open={openMoveDiscussionDialog}
          onClose={onCloseMoveDiscussionDialog}
          aria-labelledby="alert-dialog-title"
          aria-describedby="alert-dialog-description">
          ...
          <DialogContent>
            <List>
              {folders
                .filter(x => x.id != currentDiscussion.folderId)
                .sort(compareFolders)
                .map((folder, ix) => {
                  return (
                    <ListItem
          ...
          <DialogActions>
            <Button
              onClick={onCloseMoveDiscussionDialog}
              color="primary">
              Cancel
            </Button>
          </DialogActions>
